...
const WRITE_SEMANTICS = Object.freeze({
  IDEMPOTENT_WRITE: "IDEMPOTENT_WRITE",
  NON_IDEMPOTENT_WRITE: "NON_IDEMPOTENT_WRITE",
});
const AUTOMATION_ACTION_WRITE_SEMANTICS = Object.freeze({
  SEND_MESSAGE: WRITE_SEMANTICS.NON_IDEMPOTENT_WRITE,
  SFS_COMMENT_POST: WRITE_SEMANTICS.NON_IDEMPOTENT_WRITE,
  FOLLOW_BACK: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
  FOLLOW_FAN: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
  UNFOLLOW_FAN: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
  SFS_FOLLOW_TARGET: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
  SFS_UNFOLLOW_TARGET: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
  LIKE_POST: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
  SFS_LIKE_COMMENT: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
  DELETE_MESSAGE: WRITE_SEMANTICS.IDEMPOTENT_WRITE,
});
function automationActionWriteSemantics(actionType) {
  const key = String(actionType || "").trim().toUpperCase();
  // Unknown future Automation actions fail safe as non-idempotent until the
  // backend registry explicitly proves desired-state/idempotent semantics.
  return AUTOMATION_ACTION_WRITE_SEMANTICS[key] || WRITE_SEMANTICS.NON_IDEMPOTENT_WRITE;
}
...
module.exports = {
  FAILURE_CATEGORIES,
  WRITE_SEMANTICS,
  AUTOMATION_ACTION_WRITE_SEMANTICS,
  automationActionWriteSemantics,
