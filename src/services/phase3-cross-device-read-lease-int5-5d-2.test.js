...
test("INT5.5D-2 SFS and Subscriber jobs opt into the same server-wide causal read lease", () => {
  ...
  assert.match(jobs, /sfs_target_discovery:\s*"sfs_target_discovery"/);
  assert.match(jobs, /subscriber_directory_scan:\s*"subscriber_directory_page"/);
...
test("INT5.5D-2 lease purpose map remains exact and rejects cross-purpose substitution", () => {
  const jobs = source("src/services/job-lease-service.js");
  assert.match(jobs, /const expected = FAN_OBSERVATION_READ_PURPOSE_BY_JOB_KEY\[String\(job\?\.jobKey \|\| ""\)\] \|\| null/);
  assert.match(jobs, /requested !== expected/);
