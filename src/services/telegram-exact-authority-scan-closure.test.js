...
test("F44 runtime eligibility separates exact discovery from resource claim limits", () => {
  ...
  assert.match(eligible, /providerOperationalBackfillReady/);
  assert.match(eligible, /listCurrentIncompleteSourceAccountsForCreators/);
  assert.match(eligible, /listProviderOperationalAccountsForCreators/);
  assert.match(eligible, /scanActiveFollowupIntents/);
  assert.doesNotMatch(eligible, /findPendingModelInstructionAnchors|scanIncompleteTelegramSources/, "runtime discovery must not reconstruct current provider work from history");
...
test("retirement uses exact future-capability retention authority instead of model-obligation sampling", () => {
  ...
  assert.match(provider, /requireProviderOperationalBackfillReady/);
  assert.match(provider, /listProviderOperationalDebtForAccount/);
  assert.match(provider, /findStandaloneIncompleteSource/);
  assert.match(provider, /telegramInboundEvent\.findFirst/);
  assert.doesNotMatch(provider, /scanIncompleteTelegramSources|findCancelledModelInstructionFollowupDebt/, "retirement hot path must not scan historical provider evidence");
});

test("exact helper retains only bounded current/runtime primitives and historical authority exports are retired", () => {
  ...
  assert.match(helper, /async function scanActiveFollowupIntents/);
  assert.match(helper, /async function fetchAccountRowsByIds/);
  assert.match(helper, /kind:\s*\{\s*in:\s*\["REFERENCE", "MANUAL_REMINDER", "AUTO_REMINDER", "CANCELLATION"\]/);
  assert.doesNotMatch(helper, /findPendingModelInstructionAnchors|findPendingTaskAnchors|findCancelledModelInstructionFollowupDebt|findCancelledTaskFollowupDebt|findConfirmedTelegramProjectionDebt|scanIncompleteTelegramSources/);
});
