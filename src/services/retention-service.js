...
const { DEFAULT_BATCH_SIZE, defaultRetentionSettings, normalizeRetentionSettings, retentionSchema } = require("./retention-policy-definition");
const { createHash } = require("node:crypto");
const RETENTION_BATCH_BUDGET = 4;
...
async function getRetentionSettings({ db = prisma } = {}) {
  ...
  return {
    ...
    policyHash: createHash("sha256").update(JSON.stringify(settings)).digest("hex"),
...
async function resolveSweepConfig(overrides = {}) {
  if (overrides.policySettings) return normalizeRetentionSettings(overrides.policySettings);
  const current = await getRetentionSettings();
  if (!current.ok) throw Object.assign(new Error("Retention policy unavailable"), { code: "RETENTION_POLICY_UNAVAILABLE" });
...
async function renewRetentionSweepLease({ db = prisma, ownerToken, fallbackNow = new Date(), leaseMs = RETENTION_LEASE_MS } = {}) {
  ...
  return runDbTransaction(db, async (tx) => {
    ...
    if (typeof tx.$queryRawUnsafe === "function") await tx.$queryRawUnsafe('SELECT "key" FROM "RetentionSweepLease" WHERE "key"=$1 FOR UPDATE', RETENTION_LEASE_KEY);
    const authorityNow = await dbAuthorityNow({ db: tx, fallbackNow });
    const leaseUntil = new Date(authorityNow.getTime() + Math.max(60_000, Number(leaseMs) || RETENTION_LEASE_MS));
    const result = await tx.retentionSweepLease.updateMany({
      where: { key: RETENTION_LEASE_KEY, ownerToken, completedAt: null, leaseUntil: { gt: authorityNow } },
...
async function finalizeRetentionSweepLease({ db = prisma, ownerToken, outcome, error = null, fallbackNow = new Date(), onFinalize = null } = {}) {
  ...
  return runDbTransaction(db, async (tx) => {
    ...
    const owned = Number(result?.count || 0) === 1;
    if (owned && onFinalize) await onFinalize(tx, authorityNow);
    return owned;
  });
}
...
async function deleteByIdsInBatches({ model, modelName, db = prisma, where, orderBy, batchSize, label, maxBatches = RETENTION_BATCH_BUDGET }) {
  model = model || db[modelName];
  let total = 0;
  let batches = 0;
  let hasMore = false;
  const finiteMax = Math.max(1, Math.min(RETENTION_BATCH_BUDGET, Math.floor(Number(maxBatches) || RETENTION_BATCH_BUDGET)));
  for (;;) {
    ...
    const remove = tx => (modelName ? tx[modelName] : model).deleteMany({ where: { AND: [where, { id: { in: rows.map((row) => row.id) } }] } });
    const result = await runRetentionMutation(remove, db);
...
async function runTeamProjectionRetentionCompaction({ db = prisma, authorityNow = new Date(), detailDays = 180, batchSize = DEFAULT_BATCH_SIZE, agenciesPerBatch = TEAM_PROJECTION_RETENTION_AGENCIES_PER_BATCH } = {}) {
  ...
  return runMaintenanceLane({
    ...
    work: async ({ claim, heartbeat }) => {
      ...
      let deleted = 0;
      let hasMore = rows.length >= Math.max(1, Math.min(50, Math.floor(Number(agenciesPerBatch) || TEAM_PROJECTION_RETENTION_AGENCIES_PER_BATCH)));
      const details = [];
...
async function deleteFreeOrganicLedgerNoise({ batchSize, olderThan }) {
  ...
  return { label: "creatorSubscriptionLedger.free_organic_noise", deleted: total, hasMore: total >= batchSize * RETENTION_BATCH_BUDGET };
}
...
async function runTeamLedgerRetentionSweep(options = {}) {
  ...
  const [result, tipResult] = await Promise.all([
    runRetentionMutation(tx => gcTeamLedgers({
      db: tx,
      now: authorityNow,
      olderThanMs: cfg.teamMoneyRawDetailDays * DAY_MS,
      limit: cfg.batchSize,
    })),
    runRetentionMutation(tx => purgeExpiredTipLedger({
      db: tx,
...
async function runAutomationRetentionSweep(options = {}) {
  ...
  out.push(await deleteByIdsInBatches({
    modelName: "automationMonthlyAggregate",
    batchSize: cfg.batchSize,
...
async function runDialogIntelligenceRetentionSweep(options = {}) {
  ...
  const out = [];
  out.push(await deleteByIdsInBatches({
    modelName: "dialogScanChunkCommit",
    batchSize: cfg.batchSize,
...
async function runAnalyticsExecutionRetentionSweep(options = {}) {
  ...
  let ingestDeleted = 0;
  for (let batch = 0; batch < RETENTION_BATCH_BUDGET; batch += 1) {
    const deleted = Number(await runRetentionMutation(tx => tx.$executeRawUnsafe(`
  ...
  items.push({ label: `analyticsIngestBatch.earnings_${cfg.analyticsIngestBatchDays}d`, deleted: ingestDeleted, hasMore: ingestDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({ label: `jobInstance.fetch_earnings_${cfg.analyticsJobInstanceDays}d`, deleted: jobsDeleted, hasMore: jobsDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({ label: `jobInstance.fetch_earnings_unproven_${cfg.analyticsJobInstanceDays}d`, deleted: unprovenEarningsJobsDeleted, hasMore: unprovenEarningsJobsDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({ label: `creatorNotificationScanItem.audit_${cfg.analyticsNotificationScanAuditDays}d`, deleted: notificationAuditDeleted, hasMore: notificationAuditDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({ label: `analyticsIngestBatch.non_earnings_${cfg.analyticsNonEarningsIngestBatchDays}d`, deleted: nonEarningsIngestDeleted, hasMore: nonEarningsIngestDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({ label: `jobInstance.analytics_non_earnings_${cfg.analyticsNonEarningsJobDays}d`, deleted: nonEarningsJobsDeleted, hasMore: nonEarningsJobsDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({ label: `analyticsCollectionDemand.completed_${cfg.analyticsDemandHistoryDays}d`, deleted: demandsDeleted, hasMore: demandsDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({ label: `analyticsCollectionDemand.quarantined_${cfg.analyticsDemandHistoryDays}d`, deleted: quarantinedDemandsDeleted, hasMore: quarantinedDemandsDeleted >= batchSize * RETENTION_BATCH_BUDGET });
  ...
  items.push({
    ...
    deleted: proofsDeleted, hasMore: proofsDeleted >= batchSize * RETENTION_BATCH_BUDGET,
  });
...
async function runRetentionSweep(options = {}) {
  ...
  let lease = options.claimedLease || null;

  if (useCoordination && !lease) {
    try {
  ...
  let thrown = null;
  try {
    const policy = options.policySettings ? { ok: true, settings: options.policySettings } : await getRetentionSettings();
    if (!policy.ok) throw Object.assign(new Error("Retention policy unavailable"), { code: "RETENTION_POLICY_UNAVAILABLE" });
    laneOptions.policySettings = policy.settings;
    const settled = await withRetentionWork(laneOptions.retentionOwnerToken, () => Promise.allSettled(lanes.map(([, run]) => run(laneOptions))), options.actorGuard || null);
    const laneErrors = [];
    const laneResults = {};
    let totalDeleted = 0;
    ...
    if (useCoordination && lease?.acquired) {
      ...
      try {
        const finalized = await finalizeRetentionSweepLease({ db: prisma, ownerToken: lease.ownerToken, outcome, error: thrown || (report?.laneErrors?.length ? JSON.stringify(report.laneErrors) : null), onFinalize: options.onFinalize ? (tx, now) => options.onFinalize(tx, now, report, thrown) : null });
