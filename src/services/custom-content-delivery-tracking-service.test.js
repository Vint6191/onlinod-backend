...
function fixture() {
  ...
  return { order, submission, db: commitDatabaseFixture(db), audits, receipts };
}
...
test("actual outgoing media advances partial delivery and completes only after all approved media were sent", async () => {
  ...
  const first = await recordCustomDeliverySend({ agencyId: "agency-1", actorMemberId: "member-1", creatorId: "creator-1", dialogId: "777", messageId: "m-1", mediaIds: ["9001"], priceCents: 1500, occurredAt: "2026-08-22T10:10:00Z", enforceAccess: false, db: commitDatabaseFixture(db) });
  ...
  const second = await recordCustomDeliverySend({ agencyId: "agency-1", actorMemberId: "member-1", creatorId: "creator-1", dialogId: "777", messageId: "m-2", mediaIds: ["9002"], priceCents: 500, occurredAt: "2026-08-22T10:12:00Z", enforceAccess: false, db: commitDatabaseFixture(db) });
...
test("message replay is idempotent and does not double-count offered price", async () => {
  const { order, db } = fixture();
  const input = { agencyId: "agency-1", creatorId: "creator-1", dialogId: "777", messageId: "m-1", mediaIds: ["9001"], priceCents: 2000, occurredAt: "2026-08-22T10:10:00Z", enforceAccess: false, db: commitDatabaseFixture(db) };
...
test("idempotent replay projects original typed receipt facts instead of recomputing from a later aggregate", async () => {
  const { order, db } = fixture();
  const first = await recordCustomDeliverySend({ agencyId: "agency-1", creatorId: "creator-1", dialogId: "777", messageId: "m-stable-1", mediaIds: ["9001"], priceCents: 1500, occurredAt: "2026-08-22T10:10:00Z", enforceAccess: false, db: commitDatabaseFixture(db) });
  assert.equal(first.complete, false);
  assert.equal(first.expectedPriceCents, 2000);
  assert.equal(first.actualPriceCents, 1500);
  await recordCustomDeliverySend({ agencyId: "agency-1", creatorId: "creator-1", dialogId: "777", messageId: "m-stable-2", mediaIds: ["9002"], priceCents: 500, occurredAt: "2026-08-22T10:12:00Z", enforceAccess: false, db: commitDatabaseFixture(db) });
  assert.equal(order.status, "COMPLETED");

  const replay = await recordCustomDeliverySend({ agencyId: "agency-1", creatorId: "creator-1", dialogId: "777", messageId: "m-stable-1", mediaIds: ["9001"], priceCents: 9999, occurredAt: "2026-08-22T10:30:00Z", enforceAccess: false, db: commitDatabaseFixture(db) });
  assert.equal(replay.idempotent, true);
...
test("typed delivery receipt is durable business history even when best-effort AuditLog fails", async () => {
  ...
  const result = await recordCustomDeliverySend({
    ...
    messageId: "m-receipt", mediaIds: ["9001"], priceCents: 1500, occurredAt: "2026-08-22T10:10:00Z", enforceAccess: false, db: commitDatabaseFixture(db),
  });
...
test("receipt failure rolls back the Custom aggregate when a real transaction boundary is available", async () => {
  ...
  await assert.rejects(() => recordCustomDeliverySend({
    ...
    mediaIds: ["9001"], priceCents: 2000, occurredAt: "2026-08-22T10:10:00Z", enforceAccess: false, db: commitDatabaseFixture(db),
...
test("provider replay creates exactly one typed receipt", async () => {
  ...
  const input = { agencyId: "agency-1", creatorId: "creator-1", dialogId: "777", messageId: "m-one-receipt", mediaIds: ["9001"], priceCents: 2000, occurredAt: "2026-08-22T10:10:00Z", enforceAccess: false, db: commitDatabaseFixture(db) };
  await recordCustomDeliverySend(input);
...
test("overcharge and duplicate sends emit management audit signals only after a real confirmed outgoing", async () => {
  ...
  await recordCustomDeliverySend({ agencyId: "agency-1", actorMemberId: "member-1", creatorId: "creator-1", dialogId: "777", messageId: "m-1", mediaIds: ["9001"], priceCents: 3000, overrideReason: "fan wanted extra paid version", enforceAccess: false, db: commitDatabaseFixture(db) });
  ...
  await recordCustomDeliverySend({ agencyId: "agency-1", actorMemberId: "member-1", customOrderId: "custom-1", creatorId: "creator-1", dialogId: "777", messageId: "m-2", mediaIds: ["9001"], priceCents: 0, duplicateOverride: true, enforceAccess: false, db: commitDatabaseFixture(db) });
...
test("confirmed fan-send survives a concurrent CANCEL without resurrecting the Custom", async () => {
  ...
  await assert.rejects(
    () => projectCustomDeliveryFromTeamEvent(event, { db: commitDatabaseFixture(db) }),
  ...
  const retry = await projectCustomDeliveryFromTeamEvent(event, { db: commitDatabaseFixture(db) });
...
test("unconfirmed or non-manual events cannot establish the Custom fan-delivery fact", async () => {
  ...
  assert.equal(await projectCustomDeliveryFromTeamEvent({ ...common, eventKind: "MESSAGE_SEND_CONFIRMED", actionSource: "MANUAL", lifecycle: "ATTEMPTED" }, { db: commitDatabaseFixture(db) }), null);
  assert.equal(await projectCustomDeliveryFromTeamEvent({ ...common, eventKind: "MESSAGE_SEND_CONFIRMED", actionSource: "AUTOMATION", lifecycle: "CONFIRMED" }, { db: commitDatabaseFixture(db) }), null);
  assert.equal(await projectCustomDeliveryFromTeamEvent({ ...common, eventKind: "MESSAGE_SEND_ATTEMPTED", actionSource: "MANUAL", lifecycle: "CONFIRMED" }, { db: commitDatabaseFixture(db) }), null);
...
test("CUSTOM_MANUAL_V2 rejection settlement is replay-idempotent after writeCommitAt is cleared", async () => {
  ...
  await settleCustomManualDeliveryWithCapability(input, { db: commitDatabaseFixture(db) });
  assert.equal(getWrite().writeCommitAt, null);
  const replay = await settleCustomManualDeliveryWithCapability(input, { db: commitDatabaseFixture(db) });
...
test("CUSTOM_MANUAL_V2 settlement capability is bound to exact device/request/revision and cannot cross a retry generation", async () => {
  ...
  await assert.rejects(() => settleCustomManualDeliveryWithCapability({ ...base, deviceId: "device-b" }, { db: commitDatabaseFixture(db) }), (error) => error?.code === "CUSTOM_DELIVERY_WRITE_BINDING_MISMATCH");
  await assert.rejects(() => settleCustomManualDeliveryWithCapability({ ...base, networkRequestId: "net-v2-other" }, { db: commitDatabaseFixture(db) }), (error) => error?.code === "CUSTOM_DELIVERY_WRITE_BINDING_MISMATCH");
  await assert.rejects(() => settleCustomManualDeliveryWithCapability({ ...base, writeCommitRevision: 2 }, { db: commitDatabaseFixture(db) }), (error) => error?.code === "CUSTOM_DELIVERY_WRITE_BINDING_MISMATCH");
  await assert.rejects(() => settleCustomManualDeliveryWithCapability({ ...base, settlementToken: "wrong-token" }, { db: commitDatabaseFixture(db) }), (error) => error?.code === "CUSTOM_DELIVERY_WRITE_BINDING_MISMATCH");
  ...
  await assert.rejects(() => settleCustomManualDeliveryWithCapability(base, { db: commitDatabaseFixture(db) }), (error) => error?.code === "CUSTOM_DELIVERY_WRITE_BINDING_MISMATCH");
});
...
test("CUSTOM_MANUAL_V2 capability rejects ambiguous provider outcomes instead of fabricating terminal proof", async () => {
  ...
  await assert.rejects(() => settleCustomManualDeliveryWithCapability({ ...base, outcome: "PROVEN_NO_EFFECT", providerStatus: 409 }, { db: commitDatabaseFixture(db) }), (error) => error?.code === "CUSTOM_DELIVERY_SETTLEMENT_STATUS_AMBIGUOUS");
  await assert.rejects(() => settleCustomManualDeliveryWithCapability({ ...base, outcome: "PROVEN_NO_EFFECT", providerStatus: 408 }, { db: commitDatabaseFixture(db) }), (error) => error?.code === "CUSTOM_DELIVERY_SETTLEMENT_STATUS_AMBIGUOUS");
...
test("durable MESSAGE_SEND_CONFIRMED carries request-bound Custom override audit metadata without choosing delivery authority", async () => {
  ...
  }, { db: commitDatabaseFixture(otherDb) });
...
test("CUSTOM_MANUAL_V1 Team proof atomically settles the server-visible physical write authority", async () => {
  const { order, db } = fixture();
  const getWrite = installManualWrite(db);
  const result = await projectCustomDeliveryFromTeamEvent(guardedTeamEvent(), { db: commitDatabaseFixture(db) });
  assert.equal(result.matched, true);
  assert.deepEqual(order.deliverySentMediaIds, ["9001"]);
