...
test("a due daily cycle refreshes only the media catalog; dialogs stay realtime-first", async () => {
  const rec = recorder();
  const result = await ensureDailyVaultIntelligenceCycle({
    agencyId: "agency-1",
    creatorId: "creator-1",
  ...
  assert.equal(rec.calls.dialogs.length, 0, "daily maintenance must not start dialog discovery");
  assert.equal(result.dialogs, null);
});

test("an explicit forceDialogs request can still start head-aware recovery", async () => {
  const rec = recorder();
  const result = await ensureDailyVaultIntelligenceCycle({
    agencyId: "agency-1",
    creatorId: "creator-1",
...
test("a durably paused catalog is never restarted and does not trigger dialogs", async () => {
  const rec = recorder();
  const result = await ensureDailyVaultIntelligenceCycle({
    agencyId: "agency-1",
    creatorId: "creator-1",
...
test("daily catalog maintenance ignores dialog pause because it schedules no dialogs", async () => {
  const rec = recorder();
  const result = await ensureDailyVaultIntelligenceCycle({
    agencyId: "agency-1",
    creatorId: "creator-1",
...
test("an active catalog phase is never duplicated and does not start dialogs", async () => {
  const rec = recorder();
  const result = await ensureDailyVaultIntelligenceCycle({
    agencyId: "agency-1",
    creatorId: "creator-1",
