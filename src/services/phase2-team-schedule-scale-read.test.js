...
function rawDb() {
  ...
  const db = {
    ...
    teamShift: { async findMany(args) { shiftTake = args.take; return [{ id:"s", agencyId:"a", memberId:"m", startsAt:new Date("2026-09-09T09:00:00Z"), endsAt:new Date("2026-09-09T17:00:00Z"), timezone:"UTC", status:"PLANNED", revision:1, note:null, createdAt:new Date("2026-09-01T00:00:00Z"), updatedAt:new Date("2026-09-01T00:00:00Z"), cancelledAt:null, member:{id:"m",displayName:"Member",roleKey:"chatter",user:{name:"Member",email:"m@test"}}, creators:[{creatorId:"c",creator:{id:"c",displayName:"Creator",username:"creator",avatarUrl:null}}] }]; } },
  ...
  return { db, stats:()=>({rawCalls,shiftTake}) };
}
...
test("A15 Schedule production read uses SQL aggregates and bounded details instead of retained-range materialization", async () => {
  ...
  assert.equal(stats().shiftTake, 1001, "visible shift details are hard bounded independently of summary work");
  assert.equal(stats().rawCalls, 6, "one fixed aggregate query family replaces N materialized detail pages");
});
