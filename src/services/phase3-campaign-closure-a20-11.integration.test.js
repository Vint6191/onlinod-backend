...
async function createScope(db, s) {
  await withPhase3PostgresFixtureAuthority(db, async (tx) => {
    await tx.agency.create({ data: { id: s.agencyId, name: `A20.11 ${s.agencyId}` } });
    await tx.creatorAccount.create({ data: { id: s.creatorId, agencyId: s.agencyId, displayName: "A20.11 Creator" } });
  });
