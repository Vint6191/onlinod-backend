...
function fakeDb(seed = {}) {
  ...
  const domainWorkItems = seed.domainWorkItems === undefined ? null : (seed.domainWorkItems || []).map((row) => ({
    isOutstanding: row.isOutstanding !== undefined ? Boolean(row.isOutstanding) : (String(row.state || "READY") !== "DONE" || BigInt(row.requestedRevision || 0) > BigInt(row.completedRevision || 0)),
    ...clone(row),
  }));
