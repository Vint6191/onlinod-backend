...
test("R: backend wiring publishes access, revoke, network, key and job invalidations after mutation boundaries", () => {
  ...
  const jobPlanning = read("services/job-planning-repository.js");
  assert.match(desktop, /\/control\/events/);
  assert.match(desktop, /requireAuthDevice/);
  const controlRoute = desktop.slice(desktop.indexOf('router.get("/control/events"'), desktop.indexOf('router.post("/bootstrap"'));
  assert.match(controlRoute, /waitForDesktopControlEvents/);
  assert.match(controlRoute, /filterAuthorizedControlEventsStable\(req, result\.events\)/);
  assert.ok(controlRoute.indexOf("waitForDesktopControlEvents") < controlRoute.indexOf("filterAuthorizedControlEventsStable"));
  ...
  assert.match(jobPlanning, /JOB_AVAILABLE/);
});
