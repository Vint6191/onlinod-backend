...
const { ensureSubscriberScanDue, recoverSubscriberPublicationDebt } = require("./subscriber-directory-service");
...
async function runPhase2MaintenancePump({ db = prisma, now = new Date() } = {}) {
  ...
  phase2MaintenancePromise = (async () => {
    ...
    const lanes = [
      ...
      ["subscriberPublicationRecovery", () => recoverSubscriberPublicationDebt({ db, now, maxRuns: 8, maxStepsPerRun: 4, maxRuntimeMs: 5_000 })],
