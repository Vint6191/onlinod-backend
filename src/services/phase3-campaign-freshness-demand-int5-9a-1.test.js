...
test("INT5.9A-1 source removes fake delegated completion and makes Campaign/overview value reads freshness-cutoff aware", () => {
  ...
  assert.match(control, /fanValuesComplete: coverageMatches[\s\S]*fanValueFreshnessStatus === "COMPLETE" && campaignFrontierFreshnessStatus === "COMPLETE"[\s\S]*: fanRefreshDelegated \? false/);
