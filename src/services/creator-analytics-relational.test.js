...
test("campaign transport is page-oriented and notification engagement is accepted", () => {
  ...
  assert.match(resultService, /campaign_fan_values_batch/);
  assert.match(resultService, /ingestCampaignFanValuesBatchChunk/);
  assert.match(resultService, /ingestCampaignFanValueChunk/);
  assert.match(resultService, /"likes", "comments"/);
  assert.match(resultService, /completeEarningsScan/);
  assert.match(resultService, /scanProofId:\s*dailyLedger\.scanProofId/);
  ...
  assert.ok(creatorRefreshStart >= 0 && overviewStart > creatorRefreshStart);
  assert.match(routes, /router\.post\("\/agencies\/:agencyId\/refresh", legacyStatsGone\)/);
  assert.match(routes.slice(creatorRefreshStart, overviewStart), /ensureAnalyticsFreshness\(/);
  ...
  const liveEnd = routes.indexOf("\nrouter.", liveStart + 1);
  const liveRoute = routes.slice(liveStart, liveEnd === -1 ? routes.length : liveEnd);
