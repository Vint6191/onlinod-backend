...
for(const failure of["failAudit","failOutbox","failPromotion"])test(`${failure}: ownership, key handover, sessions, epoch, receipt and outbox all roll back`,async()=>{
 const m=makeOwnershipDb({crypto:true,[failure]:true}),before=structuredClone(m.state);await assert.rejects(()=>call(m,input(true)));assert.deepEqual(m.state,before);
});
for(const[change,code]of[
 ...
 const m=makeOwnershipDb({crypto:true}),before=structuredClone(m.state),c=input(true);change(c);await assert.rejects(()=>call(m,c),e=>e.code===code);assert.deepEqual(m.state,before);
 });
...
test("release drain and serialization failure leave ownership unchanged",async()=>{
 for(const opt of["draining","serializationConflict"]){const m=makeOwnershipDb({[opt]:true}),before=structuredClone(m.state);await assert.rejects(()=>call(m),e=>opt==="draining"||e.status===409);assert.deepEqual(m.state,before);}
});
...
for(const mode of ["impersonated","expired","revoked","unbound","wrongLineage"])
 test(`ownership requires direct owner session: ${mode}`,async()=>{
  ...
  const before=structuredClone(m.state);await assert.rejects(()=>call(m,input(),extra),e=>e.code==="OWNERSHIP_DIRECT_SESSION_REQUIRED");assert.deepEqual(m.state,before);
 });
