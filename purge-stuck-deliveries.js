 ...
 *
 * Запуск:
 *   node purge-stuck-deliveries.js                 -> DRY-RUN (только покажет)
 *   node purge-stuck-deliveries.js --apply         -> реально удалит
 *   node purge-stuck-deliveries.js --days=3 --apply -> другой порог возраста
 *   node purge-stuck-deliveries.js --all --apply    -> игнорировать возраст, снести все застрявшие
 */
...
async function main() {
  ...
  const candidates = await prisma.automationDelivery.findMany({
    ...
    select: {
      ...
      payload: true, generation: true, fanId: true, targetId: true,
      status: true, sentAt: true, createdAt: true, messageId: true,
    },
  });
  ...
  const ids = partition.deletable.map((r) => r.id);
  ...
  for (let i = 0; i < ids.length; i += CHUNK) {
    ...
    const res = await prisma.automationDelivery.deleteMany({ where: {
      id: { in: chunk },
      originKind: "AUTOMATION",
      status: { in: STUCK_STATUSES },
      AND: [
