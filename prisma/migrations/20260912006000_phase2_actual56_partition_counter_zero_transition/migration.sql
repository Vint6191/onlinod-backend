-- ONLINOD Phase 2 / Actual56 final closure A follow-up.
-- Fresh-source closure defect: never materialize outstandingCount=0 while the
-- table enforces CHECK (outstandingCount > 0). The final physical membership
-- transition deletes the projection row directly; non-final transitions only
-- decrement rows whose conserved count is greater than one.
