...
const PROOF_TEST = path.join(ROOT, "src/services/phase3-provider-capacity-postgres-int5-9a-15.integration.test.js");

function fail(message, code = 3) {
  console.error(`# PHASE3_A15_POSTGRES_PROOF_FAIL ${message}`);
  process.exit(code);
}
...
function dropSchema(cli, audit, schema, schemaFile) {
  ...
  if (out.status !== 0) console.warn(`# PHASE3_A15_POSTGRES_PROOF_CLEANUP_WARN ${schema}: ${out.stderr || out.stdout}`);
}
...
(function main() {
  ...
  const cleanSchema = `onlinod_a15_clean_${nonce}`;
  const rollingSchema = `onlinod_a15_roll_${nonce}`;
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "onlinod-a15-pg-"));
  ...
  try {
    ...
    run("clean-current-proof", process.execPath, ["--test", PROOF_TEST], { DATABASE_URL: cleanUrl, ONLINOD_POSTGRES_INTEGRATION: "1" });
    ...
    run("rolling-a13-to-current-proof", process.execPath, ["--test", PROOF_TEST], { DATABASE_URL: rollingUrl, ONLINOD_POSTGRES_INTEGRATION: "1" });

    console.log(`# PHASE3_A15_POSTGRES_PROOF_JSON ${JSON.stringify({ ok: true, cleanSchema, rollingSchema, a13Cutoff: A13_CUTOFF })}`);
  } finally {
