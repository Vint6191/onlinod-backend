...
function fail(message, code = 3) {
  console.error(`# PHASE3_A17_POSTGRES_PROOF_FAIL ${message}`);
  process.exit(code);
}
...
function dropSchema(cli, audit, schema, schemaFile) {
  ...
  if (out.status !== 0) console.warn(`# PHASE3_A17_POSTGRES_PROOF_CLEANUP_WARN ${schema}: ${out.stderr || out.stdout}`);
}
...
(function main() {
  ...
  const cleanSchema = `onlinod_a17_clean_${nonce}`;
  const rollingSchema = `onlinod_a17_roll_${nonce}`;
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "onlinod-a17-pg-"));
  ...
  try {
    ...
    console.log(`# PHASE3_A17_POSTGRES_PROOF_JSON ${JSON.stringify({ ok: true, cleanSchema, rollingSchema, a13Cutoff: A13_CUTOFF })}`);
  } finally {
