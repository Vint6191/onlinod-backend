...
async function main(){
 ...
 const migrate=(schema)=>new Promise((resolve,reject)=>{
  ...
  child.once("error",reject);child.once("close",code=>{if(code===0){console.log(JSON.stringify({migration:true,schema: schema.includes(baseline)?"baseline256":"current257",ok:true}));resolve();}else reject(new Error(output));});
 });
 let db;
 const passed=[];
